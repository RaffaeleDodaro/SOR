    ...
    def poll(self):
        with self.wlock:
            if len(self.coda) >  0:
                ...
                tempoMancante = self.scadenze[min(self.scadenze, key=lambda x: self.scadenze[x] )]
                - time.time()
#
...
#
    ...
    def show(self):
        ...
        with self.wlock:
            ...
            print ("MIN:{ .2f} ".format(self.poll()),end='')

            for e in self.coda:
                print ("{}:0 ".format(e), end='')

            for e in self.scadenze:
                print("{ }:{ .3f} ".format(e,self.scadenze[e]-time.time()),end='')
