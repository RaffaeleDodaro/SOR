...
# l’estrazione è ancora in scadenza, viene estratto l’elemento destinato a scadere per ultimo. 
# La scadenza stessa non viene rispettata e l’elemento viene estratto immediatamente.
...
class Producer(Thread):
    ...
    def run(self):
        while True:
            time.sleep(random.random() * 2)
            Producer.counter += 1
            self.queue.put("E-{}.{}".format(self.name,
                                            Producer.counter), random.random()*5)
            self.queue.show()
#
# Main
#
