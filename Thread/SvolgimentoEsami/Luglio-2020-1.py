...
"""
#
# Questo è il codice del Thread Stampatore. E' prevista una unica istanza che effettua le stampe
#
"""
...
class StampaPrioritaria:
    ...
    NCODE = 3
    """
    La traccia prescrive che per le stampe a media priorità, sia possibile effettuare comunque una stampa ogni  a priorità più alta,
    ...
    def __init__(self,n):
        ...
        Esempio: attese[2] =  indica che sono state fatte  stampe consecutive
        a priorità  e  senza che sia stata mai fatta una stampa a priorità 2
        """
        self.attese = []
        """
...
class clientThread(Thread):
    ...
    def run(self):
        print(f"Stampa n. del Thread {get_ident()} con priorità {self.p}")
        self.SP.stampa(f"Stampa n. del Thread {get_ident()} con priorità {self.p}",self.p)
        count = 1
        while(True):
            count += 1
